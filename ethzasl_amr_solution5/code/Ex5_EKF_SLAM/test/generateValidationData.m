...
%tuning parameters
noLines = 10; % number of lines

g = sqrt(10.0); % validation gate

sigma_pos = .15; % uncertainty of initial position
sigma_theta = 0.2; % uncertainty of initial heading
sigma_alpha = 0.2; % uncertainty of initial landmark angle
sigma_r = 0.35; % uncertainty of initial landmark distance

sigma_u = 2.0e-2; % perturbation of control inputs
...
%Generate landmarks
M = [random('unif',-pi, pi, [1, noLines])
    random('unif',0, 2.5, [1, noLines])];
...
for i = 2:length(t)
    ...
    for j = 1:size(M,2)
        [z_gt(:,j,i), Hx_gt(:,:,i,j)] = measurementFunction(x_gt(:,i), j); %ACHTUNG: this does not save all Hx!
    end
end
figure(1); cla;
...
x(1:2,1) = x_gt(1:2,1) + random('norm', 0, sigma_pos, size(x_gt(1:2,1)));
x(3,1) = x_gt(3,1) + random('norm', 0, sigma_theta, size(x_gt(3,1)));

%Add noise on landmarks in state
x(8:end,:) = x_gt(8:end,:) + repmat([sigma_z_alpha*randn(1), sigma_z_r*randn(1)]', (size(x,1)-7)/2, size(x,2));

%Initializing 2 landmarks as fixed
x(4:7) = x_gt(4:7);
...
u = u_gt + random('norm', 0, sigma_u, size(u_gt));
z = z_gt + random('norm', 0, sigma_z_r, size(z_gt));
...
save data/validationData.mat x_gt Fx_gt Fu_gt u_gt z_gt Hx_gt M R g Q R u z P x b
%ENDRM
