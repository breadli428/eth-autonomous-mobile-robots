...
% from the goal.
%STARTRM
costs(goalIdx(1), goalIdx(2)) = 0;      % set cost of first node to 0.0
%ENDRM
%STARTUNCOMMENT
...
%STARTRM
% In order to compute the distance function for every cell in the map, we
% expand the wavefront until the heap is empty, meaning there are no nodes
% to expand any more.
while ~isempty(heap)
%ENDRM
%STARTUNCOMMENT
...
% minimum cost until we arrive at the goal node.
while any(idx ~= goalIdx)
%ENDRM
%STARTUNCOMMENT
% % At every step, go to the predecessor of the current node, that has the
