...
%DYNAMICWINDOWAPPROACH Computes commands for a differential drive robot via
...
%     minOmega = TODO
    ...
    %STARTRM
    maxVel = max(min(parameters.maxVel, robotState.vel + parameters.maxAcc*parameters.timestep), -parameters.maxVel);
    minVel = max(min(parameters.maxVel, robotState.vel - parameters.maxAcc*parameters.timestep), -parameters.maxVel);
    maxOmega = max(min(parameters.maxOmega, robotState.omega + parameters.maxOmegaDot*parameters.timestep), -parameters.maxOmega);
    minOmega = max(min(parameters.maxOmega, robotState.omega - parameters.maxOmegaDot*parameters.timestep), -parameters.maxOmega);
    %ENDRM
end
...
%             headingTerm = TODO
            ...
            %STARTRM
            headingTerm = abs(pi - abs(angleDiff(robotStopPose.heading, atan2(goalPosition.y - robotStopPose.y, goalPosition.x - robotStopPose.x))));
            %ENDRM
        else
